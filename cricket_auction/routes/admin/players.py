...
def players_by_session():
    """View players organized by session - shows session-local status"""
    if session.get('role') not in ['owner', 'admin', 'auctioneer']:
        flash('Unauthorized')
        return redirect('/')
    ...
    try:
        ...
        # For each session, get its players
        for sess in sessions:
            cursor.execute("""
                SELECT sp.*, p.player_name, p.category, p.overseas, t.team_name as sold_team_name
                ...
                ORDER BY p.player_name
            """, (sess['id'],))
            sess['players'] = cursor.fetchall()
            
            sess['stats'] = {
                'total': len(sess['players']),
                'sold': sum(1 for p in sess['players'] if p['status'] == 'sold'),
                'available': sum(1 for p in sess['players'] if p['status'] == 'available'),
                'unsold': sum(1 for p in sess['players'] if p['status'] == 'unsold')
            }
        
        # Get master pool players not in any session
        cursor.execute("""
            SELECT p.*, ap.base_price, ap.status as master_status
            FROM players p
