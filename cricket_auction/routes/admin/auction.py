...
@bp.route('/auction')
def auction_room():
    """Main auction room — REQUIRES active session, shows ONLY that session's players and teams"""
    if not session.get('user_id'):
        return redirect('/')
    ...
    try:
        ...
        # === CURRENT PLAYER LOGIC (session-scoped from auction_sessions table) ===
        current_player = None
        current_bid = 0
        skip_votes = []
