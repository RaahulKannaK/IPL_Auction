...
def auction_room():
    ...
    try:
        # FIX: Look for live/paused auction first
...
def start_auction():
    ...
    # Clear cache when auction starts
    clear_cache('auction:status')
    clear_cache('auction:active')
...
def get_status():
    ...
    # Cache auction status for 5 seconds — reduces DB hits for polling
    cache_key = f'auction:status:{auction_id or "active"}'
    ...
    # Use 5 second cache for status polling — safe for auction display
...
def place_bid():
    ...
    # Clear cache after bid — force fresh data
